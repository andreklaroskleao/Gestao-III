import { collection, addDoc, doc, updateDoc, getDocs, query, serverTimestamp, onSnapshot } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
export async function createDoc(collectionRef, payload) {
  const data = { ...payload };
  if (!('createdAt' in data)) data.createdAt = serverTimestamp();
  if (!('updatedAt' in data)) data.updatedAt = serverTimestamp();
...
export function subscribeCollection(collectionName, queryConstraints = [], callback) {
  ...
  return onSnapshot(q, (snap) => callback(snap.docs.map((row) => ({ id: row.id, ...row.data() }))));
}
